...
class OrderAdmin(ForeignKeyModelAdmin):
    form = ModelWithOrganisationForm
    list_display = [
        'get_order_name', 'beverages_only', 'created_timestamp',
    ...
    def get_fieldsets(self, request, obj=None):
        return (
            (None, {'fields': [
                'beverages_only', 'customer_number', 'note', 'status', 'user'
            ]}),
        )
    ...
    def get_list_filter(self, request):
        ...
        return ['beverages_only', 'status', 'user', 'user__org']
...
class OrderItemAdmin(ForeignKeyModelAdmin):
    list_display = ['get_menu_name', 'get_order_name', 'quantity', 'special_requests']
